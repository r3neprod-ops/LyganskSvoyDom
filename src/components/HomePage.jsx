...
const directions = [
  { title: 'Новостройки', text: 'Подберём актуальные варианты по бюджету и срокам.', action: 'Подробнее' },
  { title: 'ЖК-партнёры', text: 'Соберём shortlist комплексов с понятными параметрами.', action: 'Подробнее' },
      ...
      <header className="sticky top-0 z-50 border-b border-line bg-white/90 backdrop-blur">
        <Container className="flex h-16 items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 md:hidden">
            <Button className="px-3 py-2 text-xs" onClick={() => setLeadOpen(true)}>
              Подбор
            </Button>
                ...
                <Reveal>
                  <div className="rounded-2xl border border-line bg-white p-4 shadow-soft">
                    ...
                    <div className="mt-4 grid gap-2">
                      <a href={brand.telegramUrl} target="_blank" rel="noreferrer" className="rounded-xl border border-line px-3 py-3 text-sm font-medium hover:bg-slate-50">
                        Telegram
                      </a>
                      <a href={brand.whatsappUrl} target="_blank" rel="noreferrer" className="rounded-xl border border-line px-3 py-3 text-sm font-medium hover:bg-slate-50">
                        WhatsApp
                      </a>
                      <a href={`tel:${brand.phoneHref}`} className="rounded-xl border border-line px-3 py-3 text-sm font-medium hover:bg-slate-50">
                        {brand.phoneDisplay}
                      </a>
                    </div>
                  </div>
            ...
            <Carousel>
              {complexes.map((complex) => (
                <Card key={complex.id} className="min-w-[290px] snap-start sm:min-w-[360px]">
                  <ImageSlot slotKey={complex.imageSlotKey} className="h-40" />
                  ...
                  <a href={complex.url} className="mt-4 inline-block text-sm font-semibold text-accent">
                    Подробнее
                  </a>
                </Card>
              ))}
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {reviews.map((review) => (
                <Reveal key={review.id}>
                  <Card>
                    <p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent">{review.sourceLabel}</p>
