...
import SlotDebug, { SlotDebugProvider, SlotDebugToggle } from './slots/SlotDebug';
import {
  BackgroundSlot,
  Badge,
  Button,
    ...
    <SlotDebugProvider>
      ...
      <main>
        <section id="hero" className="pt-6 sm:pt-8">
          <Container>
            <SlotDebug kind="bg" slotKey="hero-bg" className="rounded-2xl">
        ...
        <Section>
          <Container>
            <SlotDebug kind="bg" slotKey="telegram-cta" className="rounded-2xl">
            ...
            <Carousel>
              {complexes.map((complex) => (
                <Card key={complex.id} className="min-w-[290px] snap-start overflow-hidden p-0 sm:min-w-[360px]">
                  <SlotDebug kind="image" slotKey={`complex-${complex.id}`} className="h-44 bg-gradient-to-br from-slate-100 via-slate-50 to-white">
                    <ImageSlot slotKey={`complex-${complex.id}`} className="h-full w-full" />
                  </SlotDebug>
                  <div className="p-5">
                ...
                <Reveal key={review.id}>
                  <Card>
                    ...
                    <div className="mt-4 flex items-center gap-3">
                      <SlotDebug kind="avatar" slotKey={`review-${review.id}-avatar`} className="h-8 w-8 rounded-full">
                        <ImageSlot slotKey={`review-${review.id}-avatar`} className="h-full w-full rounded-full" />
                      </SlotDebug>
                      <p className="text-xs font-medium uppercase tracking-[0.12em] text-muted">{review.name}</p>
                    </div>
                  </Card>
                </Reveal>
              ))}
