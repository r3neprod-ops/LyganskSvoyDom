...
export function Button({ as = 'button', children, className = '', variant = 'primary', ...props }) {
  ...
  const variants = {
    primary: 'bg-accent text-white shadow-soft hover:-translate-y-0.5 hover:shadow-lift focus:ring-accent',
    secondary: 'border border-line bg-white text-text hover:-translate-y-0.5 hover:shadow-soft focus:ring-slate-300',
...
export function IconButton({ as = 'a', children, className = '', ...props }) {
  ...
  return (
    <Comp
      className={`inline-flex h-10 w-10 items-center justify-center rounded-xl border border-line bg-white text-xs font-semibold shadow-soft transition hover:-translate-y-0.5 ${className}`}
      {...props}
    >
      {children}
    </Comp>
...
export function BackgroundSlot({ backgroundKey, className = '', children }) {
  return (
    <div className={`relative overflow-hidden rounded-2xl border border-line ${className}`} data-background-key={backgroundKey}>
      {/* Insert hero/section background-image here by backgroundKey when assets are available. */}
      <div className="absolute inset-0 bg-gradient-to-r from-white/90 via-white/70 to-sky-50/50" />
